...
export class DanielewalletService {
  ...
  updateBitCoinRates() {
    this.http
      .get<BitCoinRate>('https://api.coindesk.com/v1/bpi/currentprice.json')
      .subscribe((data) => {
        if (this.bitCoinRates.length > 0) {
          let length = this.bitCoinRates.length;
          this.diff =
            data.bpi.USD.rate_float -
            this.bitCoinRates[length - 1].bpi.USD.rate_float;
        }
        this.bitCoinRates.push(data);
        // console.log(this.bitCoinRates);
      });
    this.http
      .get<BrlBitCoinRate>(
      ...
      .subscribe((data) => {
        ...
        //console.log(this.brlBitCoinRates);
      });
  }
  ...
  getBtcInBRL() {
    ...
    if (length > 0) {
      return (
        this.bitCoins / this.brlBitCoinRates[length - 1].bpi.BRL.rate_float
      );
    } else {
      return 0;
    }
